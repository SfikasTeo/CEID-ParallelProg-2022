...
{
	int provided;
	MPI_Init_thread(&argc , &argv, MPI_THREAD_FUNNELED, &provided ); //Initializes the MPI environment

	if ( provided < MPI_THREAD_FUNNELED) perror("Error: MPI threading level is not enough"), MPI_Abort(MPI_COMM_WORLD,1);
		...
		int ntrials = 64*1024;		// number of trials (problem dimension)
		int nvars = 32;	
   		unsigned long funevals = 0;


		double t0, t1;				//timers
		int i, threads=1;			//main counter
		...
		//important instruction is that mpi basically deleted the added needed arguments for mpiexec -n # from the argument list.
		//Thats why we also here use argc > 2
		...
		#pragma omp parallel
		{
			...
			unsigned short randBuffer[3] = {0,0,ltimeSeed+rank+omp_get_thread_num()};
			...
			for (trial = rank; trial < ntrials; trial+=size) {
				...
				if (fx < l_best_fx) {
					l_best_trial = trial; //assign trial that gave the absolute minimum
					l_best_jj = jj;	//assign number of iterations on that trial
					l_best_fx = fx;	//assign absolute minimum value
					for (i = 0; i < nvars; i++) //nvars is just 32 --> no need for parallelization
						l_best_pt[i] = endpt[i]; //initialize point on which the absolute minimum was found
				}
			}//end of omp for

			// Reduction of best trial between threads
			#pragma omp critical //creates a critical section to avoid race conditions between the threads
			{
				if (l_best_fx < best_fx) {
					//gather best iteration of threads
				...
				// critical reduction of funevals
				funevals += l_funevals; 
			}
		}//end of parallel 
		...
		if ( best_fx == global_data[1] ){
			...
			FILE* output;
			char* stringOutput;
			if ( 0 > asprintf(&stringOutput,"MPI+OMP.%d.txt", size*threads )) perror("String formatting failed"), MPI_Abort(MPI_COMM_WORLD,1);
			...
			fprintf(output,"Total number of processes used = %d. Threads used per process = %d\n", size, threads);
